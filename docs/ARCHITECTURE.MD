...
- In `Lit (IBL baked)` mode, an optional isolation toggle hides non-target meshes per bake pass to avoid cross-mesh shadow contribution.
- Rewrites exported materials as Unlit for Bake & Export.
